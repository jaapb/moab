...
let%server best_matched_language () =
  ...
  let rec aux = function
    | (l, _) :: tl -> (try Moab_i18n.guess_language_of_string l
                       with Moab_i18n.Unknown_language _ -> aux tl)
    | [] -> Moab_i18n.default_language
  in
  aux lang

let%server update_language lang =
  let language  = Moab_i18n.string_of_language lang in
  let myid_o    = Os_current_user.Opt.get_current_userid () in
  (* Update the server and client values *)
  Moab_i18n.set_language lang;
  ignore [%client (Moab_i18n.set_language ~%lang : unit)];
  ...
  Os_session.on_start_connected_process
    (fun userid ->
       ...
       let%lwt language = match%lwt Os_user.get_language userid with
         | Some lang ->
           Lwt.return (Moab_i18n.guess_language_of_string lang)
         | None ->
           let%lwt best_language = best_matched_language () in
           ignore
             (Os_user.update_language
                ~userid
                ~language:(Moab_i18n.string_of_language best_language));
           Lwt.return best_language
       in
       Moab_i18n.set_language language;
       ignore [%client (Moab_i18n.set_language ~%language : unit)];
       Lwt.return_unit)
